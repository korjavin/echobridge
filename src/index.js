...
async function getUserId(userAccessToken) {
    const { data } = await axios.get(`https://api.amazon.com/user/profile?access_token=${userAccessToken}`);
    return data.user_id;
}
...
async function sendProactiveEvent(lwaToken, userId, message, requestId) {
    const event = {
        ...
        localizedAttributes: [{
            ...
            'spokenInfo': {
                'content': [{
                    'locale': 'en-US',
                    'text': `You have a new message from EchoBridge.`
                }]
            }
        }],
        relevantAudience: {
    ...
    await axios.post(ALEXA_EVENTS_ENDPOINT, event, {
        headers: {
